...
You will complete a **RESTful API** in Spring Boot that performs CRUD operations on a **User** resource (modelled after [JSONPlaceholder users](https://jsonplaceholder.typicode.com/users)). The project already has User entity, repository, service, and controller structure; you will implement the controller endpoints with the correct HTTP methods, path variables, request body, and status codes.
...
**Hoppscotch:** Use the [Hoppscotch Browser Extension](https://hoppscotch.io/) and set Interceptor to **Browser Extension** so localhost requests work.
